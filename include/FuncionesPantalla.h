...
class FuncionesPantalla
{
    ...
    ///Funciones útiles para SDL
         ...
         * @return          Si fue posible cargar la imagen
         */
        SDL_Surface* cargarImagenSinDesechar(string nombre);

        bool ponerImagen(SDL_Surface* Pantalla, SDL_Surface* image, int X, int Y);

        SDL_Texture* cargarTextura(string file, SDL_Renderer *ren);

        SDL_Texture* renderizarTexto(string message, string fontFile, SDL_Color color, int fontSize, SDL_Renderer *renderer);
